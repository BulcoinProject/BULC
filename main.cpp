...
#if defined(NDEBUG)
# error "Bulcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xcbcc788a6500f6c78b6eca507a81f0d122ffcb2f6f340b9cd12b791b9fbb2522");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bulcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bulcoin Signed Message:\n";
...
{
    // Bulcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Bulcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Bulcoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Bulcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bulcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bulcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe9b05c242964d87fe6ffdb526b43583c22a4da672c22cad948f0e2ea54a4b442");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "16.11.2021-Bulcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04115600d0cd86c3d2ba39cc012de2750fd92f5a5a354e5456aa2186c378fa57cddf8ffedeadcf14107dfc45efdbb721c1733f9525e8544ab3e0cdc45eb0092176") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1637068287;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084554035;

        if (fTestNet)  
        {
            block.nTime    = 1637068257;
            block.nNonce   = 387519368;
        }  
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x06534b8975a859fe782e6ed4c70e693b1f42ad2589e47ad3f55f9b55c82abbb0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xb8, 0xdc }; // Bulcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BulcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BulcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BulcoinMiner(CWallet *pwallet)
{
    printf("BulcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bulcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BulcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BulcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BulcoinMiner, pwallet));
}
